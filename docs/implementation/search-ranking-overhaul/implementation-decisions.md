...
- **Decision:**
  ...
  - **`rankedCandidates` loose suppression:** loose-fallback rows appear ONLY when the set contains no `.normal` candidate; `.unlikely` and `.rejected` never appear in ranked output (toggle/consumer concern, derived from the full evaluated set in SR-05/06). Collection-aware via set membership.

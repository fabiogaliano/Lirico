...
- **Decision / outcomes:**
  ...
  - **Test matrices complete:** LyricsXFoundation 68 tests cover the 20 required evaluator/ranker cases; LyricsKit 65 tests cover the 8 required provider-event/LRCLIB cases. App build clean.
...
- **REMAINING KNOWN ITEMS (intentionally NOT done in SR-08; surfaced to user):**
  ...
  - **DEC-004 — `useLocalLyricsKit` default-on:** `LyricsXPackage/Package.swift` builds against the LOCAL LyricsKit checkout by default. Before publishing/merging beyond these local branches, the LyricsKit SR-01/SR-02 changes must be published and the remote pin bumped, then revert `useLocalLyricsKit` (escape hatch: `LYRICSX_USE_REMOTE_LYRICSKIT=1`). This is a release/publish step, not in-repo cleanup.
